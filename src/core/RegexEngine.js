...
export class RegexEngine {
  ...
  _concat(exp) {
    // Iterate over Unicode code points so ε (multi-byte) is treated as one unit
    const chars = [...exp];
    let r = '';
    for (let i = 0; i < chars.length; i++) {
      r += chars[i];
      if (i + 1 < chars.length) {
        const L = chars[i], R = chars[i + 1];
        // ε is transparent to concat: never put a dot immediately before or after it
        // (ε next to another atom is handled naturally once postfix is built)
        const lEnds   = L !== '(' && L !== '|';
        const rStarts = R !== ')' && R !== '|' && R !== '*' && R !== '+' && R !== '?' && R !== 'ε';
        const lEndsEps = L === 'ε';
        // Insert dot only when L can end AND R can start, but not when L is ε
        // (ε already acts as concatenation glue itself)
        if (lEnds && rStarts && !lEndsEps) r += '.';
        // When L IS ε and R can start a new atom, we DO need the dot
        if (lEndsEps && rStarts) r += '.';
      }
    }
    return r;
  }
  ...
  _postfix(exp) {
    ...
    let out = '', stk = [];
    for (const c of [...exp]) {  // spread so ε is one token
      if (!['|', '.', '*', '+', '?', '(', ')'].includes(c)) {
        out += c;
        if (c !== 'ε') this.alpha.add(c);   // ε is NOT part of the alphabet
      } else if (c === '(') {
        stk.push(c);
      } else if (c === ')') {
  ...
  _buildNFA(pf) {
    ...
    for (const c of [...pf]) {  // spread so ε is one token
      if (!['|', '.', '*', '+', '?'].includes(c)) {
        if (c === 'ε') {
          // ε-literal: fragment whose start immediately ε-transitions to accept
          // Matches the empty string without consuming any character
        ...
        // one-or-more: like * but no skip arc from new start
        const n = stk.pop();
        const s = this._mk(), a = this._mk();
        s.add(EPS, n.start);
        this.S.get(n.accept).add(EPS, n.start); // loop back
        ...
        s.add(EPS, n.start);
        s.add(EPS, a.id);            // skip
        this.S.get(n.accept).add(EPS, a.id);
        stk.push(new NFAFragment(s.id, a.id));
      }
    }
...
export function validateRegex(s) {
  ...
  let d = 0;
  const chars = [...s];   // iterate code points so ε counts as one char
  for (let i = 0; i < chars.length; i++) {
    const c = chars[i];
    if (c === '(') d++;
    else if (c === ')') {
    ...
    // ε is always valid — no extra restrictions
  }
